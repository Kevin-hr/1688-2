...
def main():
    parser = argparse.ArgumentParser(description="Check Ozon task status")
    parser.add_argument("task_id", help="The Task ID to check")
    parser.add_argument("--watch", action="store_true", help="Watch the task status until it completes")
    ...
    try:
        ...
        if watch_mode:
             status_info = ozon_manager.poll_task_status(task_id)
        else:
